{
    ...
    for (Long64_t iEntry = 0; iEntry<10000; iEntry++){
        ...
        for (int iLayer = 1; iLayer<NLAY; iLayer++){
            for (int iHit = 0; iHit<fitD->size(); iHit++){
                ...
                if (chi2>2||nHitsS<7||fitD->at(iHit)<FDmin[iLayer]||fitD->at(iHit)>FDmax[iLayer]||layerID->at(iHit)!=iLayer) continue;
                int preBinXZ = -1;
    ...
    for (int iLayer = 1; iLayer<NLAY; iLayer++){
        ...
        canvYX->cd(i); hYX[iLayer]->Draw("COLZ");
        canvYZ->cd(i); hYZ[iLayer]->Draw("COLZ");
        canvXZ->cd(i); hXZ[iLayer]->Draw("COLZ");
    }
}
