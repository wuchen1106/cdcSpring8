...
int main(int argc, char** argv){
	...
	int nEventMax = 10;
	if (argc>=6){
	    nEventMax = (int)strtol(argv[5],NULL,10);
    }
    ...
    double mcp_xc[8][11][11];
    double mcp_zc[8][11][11];
	...
	//Prepare the Canvas for 7 z-x planes
	...
	for (int ipad = 0; ipad<7; ipad++){
	    ...
	    double y2 = (3-ir)/4.;
	    int ilayer = ipad+1;
	    p_zx[ilayer] = new TPad(Form("p_zx_%d",ipad),"p_zx",x1,y1,x2,y2);
        p_zx[ilayer]->Draw();
	...
	TEllipse * ewiret[9][11];
	TEllipse * ewiret2[9][11];
	TText * text[9][11];
	for (int lid = 0; lid<9; lid++){
    ...
    // Prepare track points on 7 z-x planes
    TMarker * point_track_zx[8];
    for (int ilayer = 1; ilayer<=7; ilayer++){
        point_track_zx[ilayer] = new TMarker(0,0,20);
        point_track_zx[ilayer]->SetMarkerColor(kRed);
        point_track_zx[ilayer]->SetMarkerSize(0.3);
    }
    // Draw wires on the 7 z-x planes
    for (int ilayer = 1; ilayer<=7; ilayer++){
        p_zx[ilayer]->cd(); // layer from 1~8
        // Background graph for z-x planes
        TGraph * gr_all = new TGraph(9*11*2,&(map_z[0][0][0]),&(map_x[0][0][0]));
        gr_all->SetTitle(Form("layer #%d and layer #%d",ilayer,ilayer+1));
        gr_all->SetMarkerColor(kWhite);
        gr_all->SetMarkerSize(0.1);
        gr_all->GetXaxis()->SetTitle("z [mm]");
        gr_all->GetYaxis()->SetTitle("x [mm]");
        gr_all->Draw("AP");
        for (int iwire = 0; iwire<11; iwire++){
            TLatex * text1 = new TLatex(map_z[ilayer][0][0]-10,map_x[ilayer][0][0]+10,Form("# %d",ilayer));
	...
	for ( int iEntry = 0; iEntry<nEventMax; iEntry++){
		if (iEntry%100==0) printf("%lf%...\n",(double)iEntry/nEntries*100);
...
//		if ((*i_driftT)[the_ihit]>80||chi2>3) continue;
		...
		for (int ihit = 0; ihit<i_driftD->size(); ihit++){
		    ...
		    l_zx[lid][wid] = new TLine();
            l_zx[lid][wid]->SetLineColor(color[wid]);
            l_zx[lid][wid]->SetLineStyle(2);
            l_zx[lid][wid]->SetX1(-chamberHL);
            l_zx[lid][wid]->SetX2(chamberHL);
            l_zx[lid][wid]->SetY1(wxhv+delta);
            l_zx[lid][wid]->SetY2(wxro+delta);
        ...
        for (int lid = 1; lid<=7; lid++){
			...
			for (int wid = 0; wid<11; wid++){
                if (l_zx[lid][wid])
                    l_zx[lid][wid]->Draw("SAME");
                if (l_zx[lid+1][wid])
                    l_zx[lid+1][wid]->Draw("SAME");
            }
        }
